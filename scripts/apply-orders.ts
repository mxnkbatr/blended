...
async function main() {
  ...
  const policies = [
    ...
    `create policy "shop_orders_read_own" on public.shop_orders for select using (auth.uid() = user_id or user_id is null)`,
    `drop policy if exists "shop_order_items_insert" on public.shop_order_items`,
    `create policy "shop_order_items_insert" on public.shop_order_items for insert with check (true)`,
    `drop policy if exists "shop_order_items_read" on public.shop_order_items`,
    `create policy "shop_order_items_read" on public.shop_order_items for select using (
      exists (
  ...
  console.log("Shop orders tables + RLS applied.");
}
