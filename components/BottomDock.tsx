...
export function BottomDock() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 md:hidden">
          ...
          <div className="pointer-events-auto flex w-full max-w-sm gap-2">
            ...
            <Link
              ...
              className={`flex items-center justify-center rounded-[1.15rem] bg-achira-cream py-3 text-[11px] font-semibold tracking-wide text-achira-blue-dark shadow-[0_10px_28px_rgba(0,0,0,0.35)] transition-transform active:scale-[0.97] ${
                count > 0 ? "flex-1" : "min-w-[11rem] px-8"
              }`}
            >
              Цаг авах
            </Link>
      ...
      <nav
        className="relative border-t border-white/[0.08] bg-achira-navy/92 pb-[env(safe-area-inset-bottom)] backdrop-blur-3xl backdrop-saturate-150 dark:bg-achira-navy/92"
        aria-label="Доод цэс"
        style={{
          ...
          {tabs.map(({ href, label, Icon }) => {
            ...
            return (
              <Link
                ...
                className={`relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 transition-opacity active:opacity-70 ${
                  active
                    ? "text-achira-cream"
